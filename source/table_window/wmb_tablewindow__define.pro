...
pro wmb_TableWindow::Evt_Context, event
        ...
        'sort_ascending': begin
            ...
            if dtype eq 0 or $
               ...
               dtype eq 11 then begin
                ...
                msgtxt = 'Sorting is supported only for basic IDL types'
                ok = dialog_message(msgtxt, dialog_parent=event.top, $
                                    /INFORMATION)
                                    
            endif else begin
                
                ; the datatype is sortable - do the sort
                
                sortindex = sort(coldata)
                
                tabledata = tabledata[sortindex]
                
                widget_control, self.wid_table, set_value = tabledata
                
                ptr_free, self.dataptr
                
                self.dataptr = ptr_new(tabledata, /NO_COPY)
                
            endelse

        end
        
        'sort_descending': begin
            ...
            if dtype eq 0 or $
               ...
               dtype eq 11 then begin
                ...
                ; reverse the index (descending sort)
...
pro wmb_TableWindow::Evt_Table, event
    ...
    case evt_type of
        ...
        'WIDGET_CONTEXT': begin
            ...
            ; handle right click events from the table
            
            ; if the user right clicks on one of the column headers, show a 
            ; context menu with options allowing the table to be sorted 
            ; in ascending or descending order
            
            self.stat_context_evt_row = event.row
            self.stat_context_evt_col = event.col
            
            if event.row eq -1 and event.col ge 0 then begin
                
                ; the user has right clicked on one of the column headers
                
                ; first, select the entire column
                
                tablewid = self.wid_table
                
                ; get the current view position (left, top)
                
                tmpview = widget_info(tablewid, /TABLE_VIEW)
                
                ; make the selection

                tmpselect = [event.col, -1, event.col, -1]
                widget_control, tablewid, set_table_select = tmpselect
                
                ; reset the view
                
                widget_control, tablewid, set_table_view = tmpview
                
                ; display the context menu

                cmenu = self.wid_context_menu
                
                widget_displaycontextmenu, tablewid, event.x, event.y, cmenu
...
pro wmb_TableWindow::Context_menu_init

    compile_opt idl2, strictarrsubs

    table_wid_id = self.wid_table
    ...
    button = widget_button(cmenu, value='Sort table descending', $
              uvalue={Object:self, Method:'Evt_Context', $
...
pro wmb_TableWindow__define
    ...
    struct = {       wmb_TableWindow,                          $
                     ...
                     stat_context_evt_row        : 0,          $
                     stat_context_evt_col        : 0,          $
                     stat_currentpath            : '',         $
                                                               $
