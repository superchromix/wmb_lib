...
pro wmb_TableWindow::Evt_TLB, event
    ...
    if tag_names(event, /STRUCTURE_NAME) eq 'WIDGET_BASE' then begin
        ...
        new_tlbxsize = self.con_tlb_minxsize > bsize[0] < self.con_tlb_maxxsize
        new_tlbysize = self.con_tlb_minysize > bsize[1] < self.con_tlb_maxysize 
        
        ; determine how much the tlb size needs to be changed
        
        deltax = new_tlbxsize - self.stat_tlb_xsize
        deltay = new_tlbysize - self.stat_tlb_ysize
        ...
        self -> Update_Widget_Geo, tlb_deltax = deltax, tlb_deltay = deltay
...
pro wmb_TableWindow::Evt_Table, event
    ...
    ; print, tag_names(event,/STRUCTURE_NAME), event
