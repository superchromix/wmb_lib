...
;    20 June 2018   Modified to work when there is only a single coordinate 
;                   in the input arrays
;-
function wmb_matchall_2d, x1, y1, x2, y2, maxdistance, nwithin, distance=distance
...
h = hist_nd(reform([reform(x2,1,n_elements(x2)),reform(y2,1,n_elements(y2))],2,n_elements(x2)), $
