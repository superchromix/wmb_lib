...
function wmb_matchallmap, ngrid, aind, usemap=map
  ; inverse of array_indices:
  tmpvar = rebin( 1#[1,product(ngrid[0:n_elements(ngrid)-2], /cumul, /int)], size(aind,/dimen), /sample)
  index = total( reform(aind * tmpvar, size(aind,/dimen)), 2, /int)
...
;-
function wmb_matchall_nd, p1, p2, maxdistance, nwithin, distance=distance
...
off1 = reform(off1, size(p1,/dimensions), /OVERWRITE)
off2 = reform(off2, size(p2,/dimensions), /OVERWRITE)
bin1 = floor(off1)
bin2 = floor(off2)
bin1 = reform(bin1, size(p1,/dimensions), /OVERWRITE)
bin2 = reform(bin2, size(p2,/dimensions), /OVERWRITE)

; calculate 1D indices
indices1 = wmb_matchallmap(ngrid, bin1)
indices2 = wmb_matchallmap(ngrid, bin2)
...
for ci=0L,ncell-1 do begin
  ...
  tmpvar = bin1+rebin(1#di,n1,ndimen,/sample)*off1
  tmpvar = reform(tmpvar, size(bin1,/dimensions), /OVERWRITE)
  b = wmb_matchallmap(ngrid, tmpvar, usemap=map)
  ...
  for k=long(om eq 0), n_elements(h2)-1 do if h2[k] gt 0 then begin
    ...
    if ndimen gt 1 then these_dist2 = total( (p2[these_points,*]-p1[these_bins,*])^2, 2) $
                   else these_dist2 = (p2[these_points,*]-p1[these_bins,*])^2
