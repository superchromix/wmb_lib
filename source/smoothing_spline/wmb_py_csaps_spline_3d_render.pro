...
;
;   wmb_py_csaps_spline_3d_render
;
;   CSAPS cubic smoothing spline
;
;cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

function wmb_py_csaps_spline_3d_render, x_input, $
                                        y_input, $
                                        z_input, $
                                        spl_breaks_x, $
                                        spl_breaks_y, $
    ...
    spl_int_bound_z_sort_ind = sort(spl_breaks_z)
    ...
    spl_interval_bounds_z_sorted = spl_breaks_z[spl_int_bound_z_sort_ind]
    ...
    tmp_bin_edges_z = spl_interval_bounds_z_sorted
    tmp_n_bins_z = N_elements(tmp_bin_edges_z) - 1
    ...
    tmp_hist_z = wmb_variable_bin_histogram(z_input, $
                                            tmp_n_bins_z, $
                                            tmp_bin_edges_z, $
                                            reverse_indices = tmp_ri_z)
    ...
    interval_index_z = lonarr(tmp_n_points)
    interval_index_z[*] = -1
    ...
    ; assign the Z interval index for each input point

    for i = 0, tmp_n_bins_z-1 do begin
        ...
        if n_ind gt 0 then begin
            ...
            tmp_ind = tmp_ri_z[si:ei]
            interval_index_z[tmp_ind] = i

        endif

    endfor

    ; add Z values at the leftmost boundary (and beyond) to the first interval
    tmpind = where(z_input lt tmp_bin_edges_z[0], n_past_left_bound)
    if n_past_left_bound gt 0 then interval_index_z[tmpind] = 0

    ; add Z values at the rightmost boundary (and beyond) to the last interval
    tmpind = where(z_input ge tmp_bin_edges_z[-1], n_past_right_bound)
    if n_past_right_bound gt 0 then interval_index_z[tmpind] = tmp_n_bins_z - 1



    f_out = double(x_input)
    f_out[*] = 0.0
    ...
    for i = 0, tmp_n_points-1 do begin
        ...
        tmpz = z_input[i]
        tmp_int_z = interval_index_z[i]
        deltaz = tmpz - spl_interval_bounds_z_sorted[tmp_int_z]
        ...
        for j = 0, 3 do begin
            for k = 0, 3 do begin
                for m = 0, 3 do begin
                    ...
                    tmp_sum += spl_coeffs[tmp_int_z, tmp_int_y, tmp_int_x, m, k, j] * deltax^(3-j) * deltay^(3-k) * deltaz^(3-m)
